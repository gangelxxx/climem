...
- **Точки расширения, ещё не сделанные** (см. [roadmap.md](roadmap.md)): `cm mcp` —
  заглушка (выход с кодом 2); локальная нейромодель в процессе (слот под ONNX в `embed/`);
  PDF импорт/экспорт — за фичей `pdf` (по умолчанию выключена).
