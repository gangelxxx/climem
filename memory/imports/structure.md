...
| `doctor.rs` | **Диагностика целостности + статистика + (под `--fix`) безопасная делегированная починка** — read-сторона зеркала к самозалечиванию `init` (диагностирует то, что починили бы `init`/`reindex`, но не дублирует логику). По умолчанию только читает: проверяет раскладку и производный `store.db` против md-правды, печатает статистику, на каждую проблему даёт точную команду-фикс; ничего не пишет без `--fix`. Ключевые проверки: парсится ли `config.json`; резолвится ли `config.data_dir` (ловит **переименованную/перемещённую** папку — ищет ОДНОГО однозначного соседа со `store.db`); есть ли и открывается ли `store.db` (ловит **удалённый** индекс); ручная синхронизация `notes`↔`notes_fts`; дрейф подписи/размерности эмбеддера (косинус молча → 0); дрейф store↔диск (строка БД без `notes/<id>.md` и наоборот); slug-коллизии; граф кода; **`note_encoding`** (тела `notes/*.md`/`imports/*` с потерянным не-ASCII — серии `?`/`U+FFFD`/невалидный UTF-8; **report-only**, потеря необратима — текст затёрла оболочка ещё до cm; фикс = переписать/`forget`); **`agent_wiring`** (привязаны ли файлы-инструкции CLAUDE.md/AGENTS.md/… и указывают ли на **СУЩЕСТВУЮЩИЙ** бинарь cm в корне — ловит «инструкции ссылаются на cm, которого нет»; через `init::layout_status`); `scaffold` (info: gitignore/CM_GUIDE). `--fix` применяет ТОЛЬКО безопасное, делегируя примитивам: создаёт недостающие подпапки и пересобирает индекс через `commands::reindex` (`--all` при дрейфе эмбеддера/рассинхроне FTS), и при однозначном переименовании переписывает `config.data_dir` (единственная запись в конфиг — `config::set_path`+`save_raw`, неизвестные ключи сохраняются; терминальна — далее повторный прогон). НЕ трогает CLAUDE.md/код, не удаляет заметки, не переписывает прочий конфиг (для раскладки/wiring — `cm init`). Вывод: JSONL `{check,status,fixable,fix}` на каждую проверку + `{stat,…}` + итог `{doctor,errors,warnings,info,fixed}`; находки — данные, поэтому **всегда exit 0**. stdin не читает; `--fix`/`--text` булевы (нет value-флагов). **`--text`** — человекочитаемый отчёт (выровненные ✓/•/⚠/✗ + блок статистики, байты в МБ) вместо JSONL, задокументированное исключение из «JSONL на stdout» (как `config`/`export`/`help`); рендер `render_text`/`summary_text`/`format_stat`/`human_bytes`. Регистрация в `main::dispatch` (обычная команда через `Ctx`) | `run`, `diagnose`/`store_checks` (+`Report`/`Finding`/`Status`), `find_renamed_data_dir`, `rewrite_data_dir`, `run_reindex`, `note_files_without_row`/`import_files_unrecorded`, `dir_has_md`/`dir_has_files`/`is_md`/`sample` |
| `util.rs` | Самоисцеляющийся тип ошибки + UTC-время без зависимостей | `AppError` (+`with_hint`), `Result`, `now`, `iso_utc`, `civil_from_days`, `preview`, **`qmark_runs`** (детектор затёртого не-ASCII: серии `?` ≥N в чистом ASCII — подпись оболочки, испортившей кодировку ДО cm; зовут doctor `note_encoding` и `remember`-гард) |
